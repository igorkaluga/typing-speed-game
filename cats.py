...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    selected_p = [s for s in paragraphs if select(s) == True]
    return selected_p[k] if len(selected_p) - 1 >= k else ''
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    return ((typed_len - incorrect) / typed_len) * 100
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    # 2(10(the amount of charachters in increments of 5) / 5) / 30 (seconds elapsed) * 60 (minute)
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        for word in valid_words:
            ...
            if diff not in wordsHash:
                wordsHash[diff] = word
        return wordsHash[min(wordsHash)] if min(wordsHash) <= limit else typed_word
...
def feline_flips(start, goal, limit):
    ...
    # BEGIN PROBLEM 6
    if start == "" or goal == "":
        if abs(len(start) - len(goal)) > limit: #if the length difference will put you out of bounds then return large numbe
            return limit + 1 # any number greater than limit
        else:
            return abs(len(start) - len(goal))
    if limit < 0: #exceeds the limit just return large number and stop comparing
        return limit + 1 # any number greater than limit
    else:
        #if the first letter if start and goal is the same then continue checking substrings
        if start[:1] == goal[:1]:
            return feline_flips(start[1:], goal[1:], limit)
        else:
            #else we add 1 up the recursive tree and compare the substrings but now we decrease the limit - 1
...
def time_per_word(words, times_per_player):
    ...
    #iter over each nested list
    #iter within it and append to retTime
    for i in range(len(times_per_player)):
        ret_time.append([times_per_player[i][j + 1] - times_per_player[i][j] for j in range(len(times_per_player[i]) - 1)])

    return match(words, ret_time)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # for each word go through each player and find the min time starting at player 0,1 ... n
    # so for each player check their time at word indecie
    # save lowest player index in tmp var
    # after checking all players add that word index to ret_list
    for i in word_indices:
        tmp_ind = 0
        tmp_score = 10e10
        for j in player_indices:
            if get_times(match)[j][i] < tmp_score:
                tmp_score = get_times(match)[j][i]
                tmp_ind = j
        ret_list[tmp_ind].append(get_words(match)[i])
